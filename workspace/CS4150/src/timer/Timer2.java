...
//		System.out.println("Is high resolution: " + Stopwatch.IsHighResolution);
//		System.out.println("Ticks per second: " + Stopwatch.Frequency);
//		System.out.println();
	...
	public static void RunExperiment(int approach)
	{
		if (approach == 1)
		{
			...
			System.out.println("Approach 1 (single binary search): " + TimeBinarySearch1(SIZE));
		}
		else if (approach == 2)
		{
			// Do REPT lookups, time each individually.  Sum the timings
			// and report an average.
			System.out.println("Approach 2 (sum of many timings): " + TimeBinarySearch2(SIZE));
		}
		else if (approach == 3)
		{
			...
			// an average.
			System.out.println("Approach 3 (time loop, subtract overhead): " + TimeBinarySearch3(SIZE));
		}
		else if (approach == 4)
		{
			...
			System.out.println("Approach 4 (time long loop, subtract overhead): " + TimeBinarySearch4(SIZE));
		}
		else if (approach == 5)
		{
			...
			System.out.println("Approach 5 (like 4 but tries all elements): " + TimeBinarySearch5(SIZE, 5));
		}
		else if (approach == 6)
		{
			...
			{
				size = size * 2;
				double currentTime = TimeBinarySearch5(size-1, 5);
				System.out.print((size-1) + "\t" + currentTime);
				if (i > 0)
				{
...
//		return msecs(sw) / REPTS;
		return (endTime - startTime)/ REPTS;
...
//		double totalAverage = msecs(sw) / REPTS;
		double totalAverage = (endTime - startTime) / REPTS;
...
//		double overheadAverage = msecs(sw) / REPTS;
		double overheadAverage = (endTime - startTime) / REPTS;

		// Display the raw data as a sanity check
		System.out.println("Total avg:    " + totalAverage);
		System.out.println("Overhead avg: " + overheadAverage);
...
//			elapsed = msecs(sw);
	...
	/// </summary>
	public static double TimeBinarySearch5(int wordCount, int wordSize)
	{
//		// ruct a sorted array
//		int[] data = new int[size];
//		for (int i = 0; i < size; i++)
...
//		}
		...
		for(int j = 0; j < wordCount; j++){
			for(int i = 0; i < wordSize; i++){
				int charInt = r.nextInt(26);
				char nextChar = (char) (97 + charInt);
				sb.append(nextChar);
			}
...
//			elapsed = msecs(sw);
		...
		double totalAverage = elapsed / repetitions / wordCount;
...
//			elapsed = msecs(sw);
		...
		double overheadAverage = elapsed / repetitions / wordCount;
