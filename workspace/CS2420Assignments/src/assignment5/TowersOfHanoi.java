...
public class TowersOfHanoi
{
    ...
    {
    	//Checks to see if the from choice is valid
		if (from != 'A' && from != 'B' && from != 'C') {
			System.out.println("\nAbort! Illegal tower char ID!\n");
			throw new Exception();
		}
		
		//Check to see if the from choice towers are empty
		if (from == 'A' && towerA.isEmpty()) {
			System.out.println("\nAbort! Illegal tower request!\n");
			throw new Exception();
		}
		if (from == 'B' && towerB.isEmpty()) {
			System.out.println("\nAbort! Illegal tower request!\n");
			throw new Exception();
		}
		if (from == 'C' && towerC.isEmpty()) {
			System.out.println("\nAbort! Illegal tower request!\n");
			throw new Exception();
		}
		
		//Check to see if the from choice tower's top disk is larger than the to choice tower's disk and 
		//that the to choice tower is not empty.
		if (from == 'A' && to == 'B' && !towerB.isEmpty() && towerA.peek()>towerB.peek()) {
			System.out.println("\nAbort! Illegal disc movement!\n");
			throw new Exception();
		}
		if (from == 'A' && to == 'C' && !towerC.isEmpty() && towerA.peek()>towerC.peek()) {
			System.out.println("\nAbort! Illegal disc movement!\n");
			throw new Exception();
		}
		if (from == 'B' && to == 'A' && !towerA.isEmpty() && towerB.peek()>towerA.peek()) {
			System.out.println("\nAbort! Illegal disc movement!\n");
			throw new Exception();
		}
		if (from == 'B' && to == 'C' && !towerC.isEmpty() && towerB.peek()>towerC.peek()) {
			System.out.println("\nAbort! Illegal disc movement!\n");
			throw new Exception();
		}
		if (from == 'C' && to == 'B' && !towerB.isEmpty() && towerC.peek()>towerB.peek()) {
			System.out.println("\nAbort! Illegal disc movement!\n");
			throw new Exception();
		}
		if (from == 'C' && to == 'A' && !towerA.isEmpty() && towerC.peek()>towerA.peek()) {
			System.out.println("\nAbort! Illegal disc movement!\n");
			throw new Exception();
		}
		
		//Check to see that the to choice is valid
		if (to != 'A' && to != 'B' && to != 'C') {
			System.out.println("\nAbort! Illegal tower char ID!\n");
			throw new Exception();
		}
    	...
    	if(from=='A' && to=='B'){
    		towerB.push(towerA.peek());
    		towerA.pop();
    		numOfMoves++;
    	}
    	else if(from=='A' && to=='C'){
    		towerC.push(towerA.peek());
    		towerA.pop();
    		numOfMoves++;
    	}
    	else if(from=='B' && to=='A'){
    		towerA.push(towerB.peek());
    		towerB.pop();
    		numOfMoves++;
    	}
    	else if(from=='B' && to=='C'){
    		towerC.push(towerB.peek());
    		towerB.pop();
    		numOfMoves++;
    	}
    	else if(from=='C' && to=='A'){
    		towerA.push(towerC.peek());
    		towerC.pop();
    		numOfMoves++;
    	}
    	else if(from=='C' && to=='B'){
    		towerB.push(towerC.peek());
    		towerC.pop();
    		numOfMoves++;
    	}
