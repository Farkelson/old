...
public class GeocacheBrowser extends JFrame {
	...
	public GeocacheBrowser() {
		...
		// Get all the caches
		try {
			allCaches = new CacheList("caches.txt");
		}
		catch (Exception e) {
			JOptionPane.showMessageDialog(null, "Error when opening cache file:\n" + e.getMessage());
			System.exit(-1);
		}
		...
		items.setListData(allCaches.select());
...
class ResultsPane extends JPanel implements ListSelectionListener {
	...
	/**
	 * Sets the cache to be displayed.  If c is null, no information is displayed.
	 * @param c
	 */
 ...
 */
@SuppressWarnings("serial")
class SearchPane extends JPanel implements ActionListener, ItemListener, DocumentListener {
	...
	public SearchPane (JList<Cache> currentCaches, CacheList allCaches) {
		...
		// Keep track of the currently-displayed caches and the list of all caches
	...
	/**
	 * This method is called when one of the controls is manipulated.
	 * We notify the cache list of the change and then update the
	...
	private void update () {
		allCaches.setTitle(title.getText());
		allCaches.setOwner((String) owners.getSelectedItem());
		allCaches.setDifficulty(Double.parseDouble((String)lowDiff.getSelectedItem()),
						  Double.parseDouble((String)highDiff.getSelectedItem()));
		allCaches.setTerrain(Double.parseDouble((String)lowTerr.getSelectedItem()),
						  Double.parseDouble((String)highTerr.getSelectedItem()));
		currentCaches.setListData(allCaches.select());
